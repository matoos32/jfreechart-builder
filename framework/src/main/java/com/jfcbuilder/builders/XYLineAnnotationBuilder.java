...
public class XYLineAnnotationBuilder implements IXYAnnotationBuilder<XYLineAnnotationBuilder> {
  ...
  /**
   * Uses {@code Arrays.binarySearch()} two times to search the source array for
   * the x1 and x2 date value. If found, replaces the values in the builder with the found
   * array index relative to the configured index range. The source time values
   * are assumed to be timestamps in milliseconds since the epoch start. It's also
   * assumed these are in ascending chronologic order. Failure to provide them in
   * sorted order will result in undefined behavior as per
